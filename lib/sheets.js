define(function(require) {
    ...
    function createNewCanvasFromImg(id) {
        ...
        canvas.width  = sheets[id].image.width;
        canvas.height = sheets[id].image.height;

        canvas.getContext('2d').drawImage(sheets[id].image, 0, 0);

        return canvas;
    }

    function resetCellToOriginal(id, offset) {
        var context = sheets[id].canvas.getContext('2d');
        
        var cx = (offset % sheets[id].wcount) * sheets[id].cwidth;
        var cy = Math.floor(offset / sheets[id].wcount) * sheets[id].cheight;


        context.drawImage(sheets[id].image,
                          cx, cy,
