define(function(require) {
    ...
    function getCanvasWithCell(id, offset) {
        ...
        drawCell(id, context, offset);

        return canvas;
    }

    function drawCell(id, ctx, offset) {
        if (!sheets[id].image.complete) return;

        var sx = offset % sheets[id].wcount;
        var sy = Math.floor(offset / sheets[id].wcount);

        ctx.drawImage(sheets[id].image, sx * sheets[id].cwidth, sy * sheets[id].cheight, sheets[id].cwidth, sheets[id].cheight, 0, 0, sheets[id].cwidth, sheets[id].cheight);
    }
