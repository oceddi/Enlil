define(function(require) {
    ...
    function waypointForward(entry) {
        ...
        for (var i = 0; i < entry.indices.length; i++) { /* Advance to next waypoint, if easily addressable. */
            if (waypoints[entry.indices[i] + amount]) { /* Are we about to set the first indice to the last waypoint? */
                ...
                if (i == entry.indices.length - 1 && entry.reverses) entry.direction = -(entry.direction);
            }
            else if (entry.reverses && (i == entry.indices.length - 1)) { /* Reverse direction. */
                reversed = true;
            }
            else if (entry.repeats) { /* Jump to start. */
                entry.indices[i] = 0;
            }
        }
    ...
    function waypointBackwards(entry) {
        ...
        for (var i = 0; i < entry.indices.length; i++) { /* Advance to next keyframe, if easily addressable. */
            if (waypoints[entry.indices[i] + amount]) { /* If advancing past first location, reverse directions. */
            ...
            else if (entry.repeats) { /* Jump to end. */
    ...
    function instantiate(id, data) {
        ...
        if (!data.automatic) pathInst[inst].stopped = true;

        /* Optional parameters */
        if (data.groups) pathInst[inst].groups = data.groups;
    ...
    function interpolate(inst, timeNow) {
        ...
        if (entry.stopped === false) {
            ...
            if (entry.direction > 0) interval = entry.intervals[entry.indices[0]];
            else interval = entry.intervals[entry.indices[1]];
            ...
            if (tDelta >= interval) {
                ...
                if (entry.indices[entry.indices.length - 1] === 0) {
                    if (!entry.closeLoop) entry.runCount++;

                    if (!entry.repeats) {
                        entry.stopped = true;
                    }
                    else if (entry.reverses && entry.closeLoop) { /* Reverse direction. */
                ...
                if (!skipAdvance) {
                    if (entry.direction > 0) {
                        if (waypointForward(entry)) { /* Reverse direction. */
                            entry.direction = -(entry.direction);
                            swapIndices(entry.indices);
                        }
                    }
                    else {
                        if (waypointBackwards(entry)) { /* Reverse direction. */
                            entry.direction = -(entry.direction);
                            swapIndices(entry.indices);
                        }
                    }
    ...
    function currentLocation(inst, t) {
        ...
        case 'linear':
            ...
            loc = {
                'x': parseInt((1 - t) * P0[0] + t * P1[0], 10),
                'y': parseInt((1 - t) * P0[1] + t * P1[1], 10),
                'gid': entry.groups ? entry.groups[entry.indices[0]] : 0
            };
            break;

        case 'quadratic':
            P2 = paths[entry.id].waypoints[entry.indices[2]]; /* Second order (quadratic) Bezier curve calcuation (3 points). */
            /* B(t) = (1−t)^2⋅P0 + 2⋅(1−t)⋅t⋅P1 + t^2⋅P2        */
            loc = {
                'x': parseInt(Math.pow((1 - t), 2) * P0[0] + 2 * (1 - t) * t * P1[0] + Math.pow(t, 2) * P2[0], 10),
                'y': parseInt(Math.pow((1 - t), 2) * P0[1] + 2 * (1 - t) * t * P1[1] + Math.pow(t, 2) * P2[1], 10),
                'gid': entry.groups ? entry.groups[entry.indices[0]] : 0
            };
            break;

        case 'cubic':
            P2 = paths[entry.id].waypoints[entry.indices[2]];
            var P3 = paths[entry.id].waypoints[entry.indices[3]]; /* Third order (cubic) Bezier curve calculation (4 points ) */
            /* B(t) = (1−t)^3⋅P0 + 3⋅(1−t)^2⋅t⋅P1 + 3⋅(1−t)⋅t^2⋅P2 + t^3⋅P3 */
            loc = {
                'x': parseInt(Math.pow((1 - t), 3) * P0[0] + 3 * Math.pow((1 - t), 2) * t * P1[0] + 3 * (1 - t) * Math.pow(t, 2) * P2[0] + Math.pow(t, 3) * P3[0], 10),
                'y': parseInt(Math.pow((1 - t), 3) * P0[1] + 3 * Math.pow((1 - t), 2) * t * P1[1] + 3 * (1 - t) * Math.pow(t, 2) * P2[1] + Math.pow(t, 3) * P3[1], 10),
                'gid': entry.groups ? entry.groups[entry.indices[0]] : 0
            };
            break;
        }
    ...
    function interpolateAll() {
        ...
        for (var i = 0; i < pathInst.length; i++) {
            locations.push(Paths.interpolate(i, timeNow));
        }

        return locations;
    }

    function changeWayPoints(id, newPoints) {
        paths[id].waypoints = newPoints;
    }

    function changeIntervals(inst, newIntervals) {
        var entry = pathInst[inst];
        entry.intervals = newIntervals;
    }
    ...
    Paths.changeWayPoints = changeWayPoints;
    Paths.changeIntervals = changeIntervals;
