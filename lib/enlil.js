define(function(require) {
    ...
    function drawFrame() {
        ...
        for (i = 1; i < actors.length; i++) { /* Draw actor with specified group? */
            if (locations[i - 1].gid) {
                var sheet = actors[i].getSheet();

                if(!sheet)continue;

                rollover = sheet.getGroupLength(locations[i - 1].gid);
                actors[i].drawGroup(locations[i - 1].gid, Enlil.animCount % rollover, locations[i - 1].x, locations[i - 1].y);
            }
            else { /* Don't use groups, just raw offset. */
                actors[i].draw(Enlil.animCount % 4, locations[i - 1].x, locations[i - 1].y);
            }
        }
    }
    ...
    function addEventHandler(node, type, handler, matchFunc) {
        Input.addHandler(node, type, handler, matchFunc);
    }
    ...
    function animFrame() {
        ...
        if(Enlil.frameCount % 5 === 0) Particles.processParticles();
