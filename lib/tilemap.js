define(function(require) {
    ...
    function init(params) {
        ...
        for (sectory = 0; sectory < sectorstall; sectory++, mhremaining -= sheight * theight) {
            ...
            for (sectorx = 0; sectorx < sectorswide; sectorx++, mwremaining -= swidth * twidth) {
                ...
                quadtree = QuadTree.instantiate(sectorx + (((idealWidth * twidth) >> 1)),
                                                sectory + (((idealHeight * theight) >> 1)),
                                                idealWidth * twidth,
                                                idealHeight * theight,
                                                4);
        ...
        for(y=0; y<sheight; y++)
        {
            for(x=0; x<swidth; x++)
            {
                ...
                if(tilex != 255 && tiley != 255)
                {
                    ...
                    /* Insert tile into collision quadtree */
                    QuadTree.addElement(quadtree, {
                        x : left,
                        y : top,
                    width : twidth,
                   height : theight
    ...
    function checkForBlockingTile(sid, x, y, width, height) {
        ...
        test.push(translateToMapCoords(x,         y));
        test.push(translateToMapCoords(x + width, y));
        test.push(translateToMapCoords(x,         y + height>>1));
        test.push(translateToMapCoords(x,         y + height));
        test.push(translateToMapCoords(x + width, y + height));

        for(var i=0; i<test.length; i++)
        {
            tilex = mapLookup.data[((test[i][1]*mw)+test[i][0])*4];    // R
            tiley = mapLookup.data[((test[i][1]*mw)+test[i][0])*4+1];  // G
    ...
    function drawToContext(sid, worldtlx, worldtly) {
        ...
        if (drawcolltests)
        {
            for(var i=0; i<colltests.length; i++)
            {
                ...
                screenctx.fillRect(colltests[i][0], colltests[i][1], 2, 2);
            }
        }

        if (drawquadtree)
        {
            QuadTree.drawNodes(sectors[sid].qt, sxoffset, syoffset, screenctx);  
        }
    }
    ...
    function physicsColliderCheck(data) {
        ...
        for (var i = 0; i < numsectors; i++) {
            if(insidesector(i, data.x, data.y)) {
                ...
                //candidates = QuadTree.findInRect2(sectors[i].qt,
                //                                 data.x,
                //                                 data.y,
                //                                 data.width,
                //                                 data.height);
                candidates = checkForBlockingTile(sectors[i].qt,
                ...
                {
                    if(Actors.collidesWithBitmap(data.actorid,
                                                 data.x+data.screentlx,
                                                 data.y+data.screently,
                                                 candidates[j].x,
                                                 candidates[j].y,
                    ...
                    {
                        /* This highlight is just for visual confirmation of collision testing... comment out later. */
