...
define(function (require) {
		...
		while(i < data.length)
		{
			...
			switch(chunkID)
			{
				case 'MThd':
					...
					if((timeDiv & 0x8000) == 0)
					{
						/* Lower 15 bits -> Pulses Per Quarter Note. */
						console.log('PPQN: ' + (timeDiv & 0x7FFF));
						ppqn = (timeDiv & 0x7FFF);
					}
					else
					{
						/* Lower 15 bits -> Frames Per Second. */
	...
	function parseMidiTrack(data) {
		...
		while(i<data.length)
		{
			...
			if(!activeNotes[midiChannel])
				activeNotes[midiChannel] = new Array(128);
				...
				case 0xF:
					switch(midiChannel)
					{
						...
						case 0xF:
							...
							switch(metaType)
							{
								case 81: /* Set Tempo - The 3 data bytes are time (in microseconds) per quarter note. */
	...
	function generate(samples, timeMarker) {
		...
		var copiedOut   = 0;
		var copyCurrent = 0;
		var sample_interval = ( 44100 * 60) / bpm; /* sample interval for 1 quarter note. */
